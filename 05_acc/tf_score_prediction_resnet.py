...
def inception(image, scope_name, reuse):
    preprocessed = tf.multiply(tf.subtract(tf.expand_dims(image, 0), 0.5), 2.0)
    arg_scope = resnet_v2.resnet_arg_scope(weight_decay=0.0)
    with slim.arg_scope(arg_scope):
        if scope_name == 'resnet_v2_50':
            logits, _ = resnet_v2.resnet_v2_50(
                preprocessed, 1001, is_training=False, reuse=reuse)
        elif scope_name == 'resnet_v2_152':
            logits, _ = resnet_v2.resnet_v2_152(
                preprocessed, 1001, is_training=False, reuse=reuse)
        elif scope_name == 'resnet_v2_101':
...
def cal_score(img_folder, ckpt_file,scope_name, csv_file, cal_attack = True):
    ...
    logits, probs = inception(image, scope_name, reuse=False)
